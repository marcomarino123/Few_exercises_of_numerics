...
def max_min_indices_symm(matrix):
    i_max,j_max,max_value=0,1,np.abs(matrix[0,1])
    for i in range(np.shape(matrix)[0]-1):
        for j in range(i+1,np.shape(matrix)[0]):
...
def Jacobi_rotation(matrix,threshold_diag):
    transformation=np.identity((np.shape(matrix)[0]))

    offset=T_dist_from_diag(matrix)
    #print("offset",offset)
    while offset>threshold_diag:
        ...
        i_max,j_max=max_min_indices_symm(matrix)
...
# Lanczos algorithm
def Lanczos_method(matrix,max_dimension,threshold_krylov,initial_vector,threshold_lanczos,threshold_diag):
    ...
    while convergence > threshold_lanczos:
        ...
        # Building Krylov space and tridiagonal matrix
        ...
        # Diagonalization with Jacobi-Rotation
        diagonal_matrix,eigenvectors=Jacobi_rotation(tridiagonal_matrix,threshold_diag)
        eigenvalues=np.diagonal(diagonal_matrix)
...
def main():
    ...
    # threshold over diagonalization in Jacobi-rotation
    threshold_diag=1.0e-6

    # Considering different energies of the defect state
    defect_state_energies=np.arange(-20,22,2) 
    ...
    for i in range(len(defect_state_energies)):
        ...
        ground_state_energies[i],ground_state_particle_densities[i,:]=Lanczos_method(hamiltonian,max_dimension,threshold_krylov,initial_vector,threshold_lanczos,threshold_diag)
